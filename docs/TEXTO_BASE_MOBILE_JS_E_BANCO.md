...
Foram criados quatro triggers. A `TRG_ATUALIZA_ESTOQUE` soma entradas e subtrai saídas após a inserção de uma movimentação, rejeitando operações que produziriam saldo negativo. Outro gatilho registra o saldo anterior e o saldo atual em `auditoria_estoque` após cada mudança de estoque. Os dois restantes impedem a edição e a exclusão das movimentações. Quando um lançamento precisa ser corrigido, o usuário deve registrar uma movimentação compensatória. Essa regra mantém a rastreabilidade das operações e permite comparar o saldo atual com o histórico.
...
A API foi compilada com o SDK .NET 10 sem erros ou avisos. Em um banco descartável foram executados dez testes de integração. Eles verificaram as preferências de tema e avatar, o bloqueio de acesso sem sessão, a restrição administrativa, a ausência de senhas nas respostas, a identidade da movimentação, a repetição segura, duas retiradas simultâneas, o saldo insuficiente, a validação de quantidades, o cálculo do orçamento, a restrição de aprovação, o conflito de versões, a rejeição de origem externa e a revogação da sessão. Uma inserção SQL direta também confirmou que a trigger altera o estoque exatamente uma vez.
